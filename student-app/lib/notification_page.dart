...
class _NotificationPageState extends State<NotificationPage> {
  ...
  // Getter to return notifications in reverse order (newest first)
  List<Map<String, dynamic>> get reversedNotifications => notifications.reversed.toList();

  void _markAsRead(int dateIndex, int messageIndex) {
    // Since we're using reversed list, we need to calculate the original index
    int originalIndex = notifications.length - 1 - dateIndex;
    var message = notifications[originalIndex]['messages'][messageIndex];
    if (!message.read) {
      setState(() {
        message.read = true;
      });
