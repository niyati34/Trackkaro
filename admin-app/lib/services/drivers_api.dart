...
class DriversApiService {
  ...
  DriversApiService({String? baseUrl, http.Client? client})
  ...
  Future<List<Map<String, dynamic>>> getAllDrivers(
    ...
    final attempts = <({String label, Future<http.Response> Function() exec})>[
      (
        ...
        exec: () async {
          try {
            ...
            final request = await client.getUrl(_uri('/get-all-drivers'));
      ...
      (
        ...
        exec: () async {
          final uri = _uri('/get-all-drivers')
    ...
    throw Exception(_composeDiagnostic('drivers', errors));
  }
  ...
  Future<void> addDriver(Map<String, dynamic> body) async {
    ...
    if (res.statusCode != 200 && res.statusCode != 201) {
      throw Exception('Failed to add driver: ${res.statusCode} ${res.body}');
    }
  }

  Future<void> updateDriver(Map<String, dynamic> body) async {
    body['organization_id'] = _normalizeOrgId(body['organization_id']);
    ...
    if (res.statusCode != 200) {
      throw Exception('Failed to update driver: ${res.statusCode} ${res.body}');
    }
  }
  ...
  Future<void> deleteDriver(
    ...
    if (res.statusCode != 200) {
      throw Exception('Failed to delete driver: ${res.statusCode} ${res.body}');
    }
  }
  ...
  Future<void> assignBusToDriver(
      ...
      required dynamic assignmentId}) async {
    final res = await _client.post(
      _uri('/assign-bus-to-driver'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'organization_id': _normalizeOrgId(organizationId),
        'driver_id': driverId,
        'assignment_id': assignmentId,
      }),
    );
    if (res.statusCode != 200) {
      throw Exception('Failed to assign bus: ${res.statusCode} ${res.body}');
    }
  }

  Future<void> revokeDriverFromBus(
      {required dynamic organizationId,
      required dynamic driverId,
      required dynamic assignmentId}) async {
    final res = await _client.post(
      _uri('/revoke-driver-from-bus'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'organization_id': _normalizeOrgId(organizationId),
        'driver_id': driverId,
    ...
    if (res.statusCode != 200) {
      throw Exception('Failed to revoke bus: ${res.statusCode} ${res.body}');
    }
  }
  ...
  String _composeDiagnostic(String entity, List<String> errors) {
    ...
    b.writeln(
        'Backend fix: allow GET with query param and avoid unconditional request.get_json() for /get-all-drivers');
    return b.toString();
  }
}
